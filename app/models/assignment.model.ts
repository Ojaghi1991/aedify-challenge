...
import {
  ...
  AssignmentIdType,
  AssignmentStatus,
} from '../types/assignment.d';

const AssignmentModel = {
  createAssignment: async (assignment: AssignmentType) => {
    const { personnel_id, project_id, role, start_time, end_time, status } =
      assignment;
    const sqlQuery =
      'INSERT INTO assignment(personnel_id , project_id , role, start_time, end_time, status) VALUES($1, $2, $3, $4, $5, $6) RETURNING *';
  ...
  updateAssignment: async (
    ...
    const sqlQuery =
      'UPDATE assignment SET personnel_id = $1, project_id = $2, role = $3, start_time = $4,end_time = $5, status = $6  WHERE personnel_id = $7 AND project_id=$8 RETURNING *';
    const result = await pool.query(sqlQuery, [
      personnel_id,
      project_id,
      role,
  ...
  getAssignment: async ({ personnel_id, project_id }: AssignmentIdType) => {
    const sqlQuery =
      'SELECT * FROM assignment WHERE personnel_id = $1 AND project_id=$2';
  ...
  removeAssignment: async ({ personnel_id, project_id }: AssignmentIdType) => {
    const sqlQuery =
      'DELETE FROM assignment WHERE personnel_id = $1 AND project_id = $2 RETURNING *';
    const result = await pool.query(sqlQuery, [personnel_id, project_id]);
    return result.rows[0];
  },
