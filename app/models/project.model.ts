...
const projectModel = {
    createproject: async (project: ProjectTypes) =>{
        ...
        const sqlQuery = "INSERT INTO projects(projectName, budget, deadline) VALUES($1, $2, $3) RETURNING *"
    ...
    updateproject: async (id: ProjectIdType, project: ProjectTypes) => {
        const {projectName, budget, deadline} = project;
        const sqlQuery = "UPDATE projects SET projectName = $1, budget = $2, deadline = $3 WHERE id = $4 RETURNING *";
        const result = await pool.query(sqlQuery, [projectName, budget, new Date(deadline).toISOString(), id ])
        return result.rows[0]
    },
