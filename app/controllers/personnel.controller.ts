...
const personnelController = {
    ...
    update: async (req: Request, res: Response) => {
        try {
            ...
            const personnel = await personnelService.updatePersonnel(personnelId as PersonnelIdType, req.body)
            
            if(personnel) {
               res.status(200).json({ personnel })
            } else {
                res.status(404).json({ message: `Unfortunately, we couldn't find personnel #${personnelId}`})
            }
    ...
    remove: async (req: Request, res: Response) => {
        try {
            ...
            if(personnel) {
                res.status(200).json({ message: `successfully remove personnel ${personnelId}` })
            } else {
                res.status(404).json({ message: `Personnel not found` })
            }
